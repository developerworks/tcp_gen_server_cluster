...
%% 用给定的名称启动全局或本地服务器,目标gen_server由目标模块给定.
%% 如果某些全局服务器已经运行, 该服务器作为本地服务器启动, 否则则作为一个新的全局服务器.
...
start(Name, TargetModule, TargetArgs, Options) ->
    ...
    case gen_server:start({global, Name}, ?MODULE, ArgsGlobalInit, Options) of
        {ok, _GlobalServerPid} = Result ->
            lager:info("[~p] ~p started as global server.", [?MODULE,Name]),
            Result;
...
%% 如果全局服务器已经运行, 启动一个本地服务器,
%% 否则返回noGlobalServerRunning
    ...
    case is_running(Name) of
        ...
        true ->
            ...
            case gen_server:start({local, Name}, ?MODULE, ArgsLocalInit, []) of
                {ok, _LocalPid} = Result ->
                    lager:info("[~p] ~p started as local server.", [?MODULE,Name]),
                    Result;
                Else ->
                    Else
            end
...
%% 链接Pid到全局服务器. 如果当前全局服务器死亡,则该链接转换为新的全局服务器
...
init({initGlobal, Name, TargetModule, TargetArgs}) ->
    ...
    lager:info("[~p] initialize callback server: ~p, arguments: ~p", [?MODULE,TargetModule,TargetArgs]),
    TargetResult = TargetModule:init(TargetArgs),
    case TargetResult of
        {ok, TargetState} ->
            State = #state{name = Name, globalServerPid = self(), targetModule = TargetModule, targetState = TargetState},
            {ok, State};
...
%% 当全局服务器死亡时由所有本地服务器调用
%% 每个本地服务器试图把自己注册为一个新的全局服务器, 但是仅有一个会注册成功为一个真正的全局服务器
...
handle_info({'EXIT', Pid, Reason}, State) when Pid == State#state.globalServerPid ->
    lager:info("Global server on ~p terminated: ~p.", [node(Pid), Reason]),
    NewGlobalServerPid = try_register_as_global_server(State#state.name, Pid),
    lager:info("New global server on ~p.", [node(NewGlobalServerPid)]),
...
%% 当某些本地服务器或其他进程死亡时由全局服务器调用.
%% 如果一个本地服务器死亡, 它的pid将从本地服务器列表删除, 并且一个对应的状态更新发送到所有其他的本地服务器.
%% 如果某些其他的链接进程死亡, 该信息被委派给目标.在使用模块的link函数的情况,它的pid将从状态中删除,同时一个对应的状态更新发送到所有其他的本地服务器.
...
terminate(Reason, State) ->
    ...
    lager:info("Server ~p stopped.", [State#state.name]).
...
code_change(OldVsn, State, Extra) ->
    lager:info("[~p] Code changed.", [?MODULE]),
    lager:info("[~p] OldVsn: ~p, State: ~p, Extra: ~p", [?MODULE, OldVsn, State, Extra]),
    {ok, State}.
...
delegate_to_target(State, TargetCall, Args) ->
    ...
    UpdateTargetStateFun =
        fun(S) ->
            %% 当 TargetStateUpdate是一个更新函数或新状态是更新目标状态
    ...
    %% 如果目标停止, 那么停止所有的本地服务器, 通过返回这个结果, 全局服服务器将会停止
...
%% 由全局服务器调用以更新所有服务器的状态,执行UpdateFun状态更新函数,
%% 获得返回的结果作为新状态,同时向所有本地服务器发送更新请求
